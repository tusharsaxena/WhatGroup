...
4. `NS.Compat.GetSpellCooldownRemaining(spellID)` — seconds left, or `0` when ready. Asked only of a spell the player has learned. It normalizes retail's `C_Spell.GetSpellCooldown` table against the legacy multi-return, floors anything at or under the 1.5s GCD to zero (otherwise casting *anything* would make an eight-hour teleport blink "on cooldown"), and reads `isEnabled == false` — which means "do not draw a cooldown", i.e. the spell is mid-cast — as ready.
