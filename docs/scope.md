...
- **Cyan `[WG]` chat prefix.** Every line the addon prints is routed through the shared `NS.PREFIX = "\|cff00FFFF[WG]\|r"` constant (aliased to a file-local `CHAT_PREFIX` in `WhatGroup.lua`) so users can identify WhatGroup output at a glance. Debug lines additionally tag `[DBG]` in orange.
