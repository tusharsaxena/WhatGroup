...
- **Localization.** English-only *content*. The addon ships a locale **shell** (`locales/enUS.lua` → `NS.L`, a metatable that returns the key when a translation is absent), and its player-facing strings — notify, popup, help, reset, group-type labels — route through `NS.L[...]` (§8.3 / WG-07). Actual translation tables are a deliberate non-goal at this size; the shell just keeps strings out of the call sites and off Blizzard `_G` globals. (Playstyle enum *values* still read Blizzard's own localized `GROUP_FINDER_GENERAL_PLAYSTYLE1..4`.)
