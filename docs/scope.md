...
- **Group-leave wipes capture state.** `GROUP_ROSTER_UPDATE` with `inGroup == false` calls `WhatGroup:WipeCapture()`, which clears `pendingInfo`, `notifiedFor`, both `captureQueue` and `pendingApplications`, and bumps `notifyGen` to cancel any in-flight `C_Timer.After` notify callback. The same method is reused on master-switch off-flip (`enabled.onChange` with `v=false`) so a pre-toggle apply can't surface a popup after the user has explicitly disabled the addon. No "remember last group" mode.
...
- **Cyan `[WG]` chat prefix.** Every user-facing line the addon prints is routed through the shared `NS.PREFIX = "\|cff00FFFF[WG]\|r"` constant (aliased to a file-local `CHAT_PREFIX` in `WhatGroup.lua`, exposed as `NS.Print`) so users can identify WhatGroup output at a glance. Debug output does **not** go to chat — it routes to the on-screen debug console (`NS.Debug` → `DebugLog.lua`); see [debug-console.md](./debug-console.md).
