...
- **LFG state mutation.** The addon never auto-applies, never declines invites, never modifies the LFG search results, and never blocks the join flow. Every hook is observation-only — `SecureHook` on `ApplyToGroup`, and a `hooksecurefunc` post-hook on `SetItemRef` that runs only when the link prefix matches `WhatGroup:`.
...
- **`SecureHook` on `ApplyToGroup`, `hooksecurefunc` on `SetItemRef`.** `SecureHook` because we only observe the apply (no need to intercept). `hooksecurefunc` is enough for `SetItemRef` because Blizzard's default already no-ops on unknown link prefixes — a `RawHook` (which earlier versions used) leaves the global function replaced by addon code, which taints the secure execution chain that the GameMenu's Logout button relies on. See [wow-quirks.md](./wow-quirks.md#hook-discipline).
