...
- **Localization.** English-only *content*. The addon ships a locale **shell** (`locales/enUS.lua` → `NS.L`, a metatable that returns the key when a translation is absent), and its player-facing strings — notify, popup, help, reset, group-type labels — route through `NS.L[...]` (localization-§3 / WG-07); the classes that stay unrouted, and why, are ratified in [`ARCHITECTURE.md` → `## Documented deviations`](./ARCHITECTURE.md#documented-deviations). Actual translation tables are a deliberate non-goal at this size; the shell just keeps strings out of the call sites and off Blizzard `_G` globals. (Playstyle enum *values* still read Blizzard's own localized `GROUP_FINDER_GENERAL_PLAYSTYLE1..4`.)
...
- **Custom hyperlinks beyond `WhatGroup:show`.** Only the one prefix is post-hooked; every other link goes through Blizzard's default `SetItemRef` exactly as if WhatGroup weren't installed.
...
- **Group-leave wipes capture state.** `GROUP_ROSTER_UPDATE` with `inGroup == false` calls `WhatGroup:WipeCapture()`, which clears `pendingInfo`, `notifiedFor`, both `captureQueue` and `pendingApplications`, and `CancelTimer`s any in-flight AceTimer notify callback (`self.notifyTimer`, WG-17). The same method is reused on master-switch off-flip (`enabled.onChange` with `v=false`) so a pre-toggle apply can't surface a popup after the user has explicitly disabled the addon. No "remember last group" mode.
- **The join trigger is dual-path, funnelled through `_TryFireJoinNotify`.** Retail fires `GROUP_ROSTER_UPDATE` and the `inviteaccepted` LFG status in either order, so a single gate on the roster transition silently missed the joins where `pendingInfo` landed second. `WhatGroup:_TryFireJoinNotify(reason)` is called from BOTH paths and gates on three conditions: `pendingInfo` set, `IsInGroup()` true, and `notifiedFor ~= pendingInfo` — the `notifiedFor` identity flag is what stops both paths firing for the same join. `wasInGroup` still bounds the roster path to the not-in-group → in-group transition, so ordinary roster churn inside an existing group does nothing. See [data-flow.md](./data-flow.md).
- **Direct `hooksecurefunc` on both `C_LFGList.ApplyToGroup` and `SetItemRef`.** Both hooks are direct Blizzard `hooksecurefunc` calls (no AceHook). AceHook's `SecureHook` / `RawHook` wrappers leave a per-invocation closure around the callback, and that closure taints the secure-execute chain — the taint surfaces later as `ADDON_ACTION_FORBIDDEN ... 'callback()'` at `Blizzard_GameMenu/Shared/GameMenuFrame.lua:69` when the player clicks Logout. `hooksecurefunc` is enough for `SetItemRef` because Blizzard's default already no-ops on unknown link prefixes; a `RawHook` (which earlier versions used) leaves the global function replaced by addon code, with the same taint outcome. See [midnight-quirks.md](./midnight-quirks.md#hook-discipline).
...
- **Parent settings category is intentionally a thin landing page.** WoW 12.0 hides parent-category widgets when the parent has subcategories, so widgets there would never display. Every actual setting lives in the General subcategory; the parent panel hosts a logo + TOC notes one-liner + the slash-command list (read live from `WhatGroup.COMMANDS`). See [midnight-quirks.md](./midnight-quirks.md#settings-api-parent-vs-subcategory).
- **`/wg config` opens the parent and unfolds the tree.** The verb calls `Helpers.OpenOptionsPanel()` — `LibKa0s-Options-1.0`'s, since the adoption — which holds the main category's own ID, runs `Settings.OpenToCategory` against the parent and then a `pcall`-wrapped `SettingsPanel:GetCategoryList():GetCategoryEntry(parent):SetExpanded(true)`. The user lands on the addon-landing page (logo + slash list) with every subcategory visible one click away in the sidebar. It refuses to open during `InCombatLockdown()` — the Settings UI uses secure templates that can taint mid-combat — and that gate lives inside `OpenOptionsPanel` rather than in the dispatcher, so every caller is refused.
- **Settings panel bodies are built lazily.** AceGUI widgets must render against a non-zero panel width, which is zero at `PLAYER_LOGIN`. Both the parent landing page and the General subcategory keep their `OnShow` body a no-op and do the actual build inside a `C_Timer.After(0, …)` hop, so no AceGUI frame is ever created inside Blizzard's secure-execute chain. The one-shot bookkeeping is the library's per-ctx `_rendered` / `_dirty` pair now; the extra frame hop is this addon's, wrapped onto the instance in `settings/OptionsSetup.lua`. The header's Defaults button builds in that same hop — partly for the taint rule, partly because a widget created during load misses the UI skins that hook `RegisterAsWidget` (options-ui-§5). See [midnight-quirks.md](./midnight-quirks.md#lazy-acegui-panel-build).
- **Cyan `[WG]` chat prefix.** Every user-facing line the addon prints is routed through one secret-safe printer — `NS.Util.print` (`LibKa0s-Core-1.0`'s, built in `core/CoreSetup.lua`), exposed as `NS.Print` — which prepends the shared `NS.PREFIX = "\|cff00FFFF[WG]\|r"` constant and `NS.SafeToString`s each argument (WG-22), so users can identify WhatGroup output at a glance and a combat-protected value can't raise in the chat path. Debug output does **not** go to chat — it routes to the on-screen debug console (`NS.Debug` → `LibKa0s-DebugLog-1.0`, wired in `core/DebugLogSetup.lua`); see [debug.md](./debug.md).
...
- User-facing behavior: [README.md](../README.md) — slash commands, FAQ, troubleshooting.
