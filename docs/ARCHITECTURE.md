...
teleport cooldown countdown at `modules/Frame.lua:441`, a 1-second `ScheduleRepeatingTimer` added
...
1. **libs/** — `LibStub` → `CallbackHandler-1.0` → `AceAddon-3.0` → `AceEvent-3.0` → `AceConsole-3.0` → `AceTimer-3.0` → `AceDB-3.0` → `AceGUI-3.0` (via its `.xml`, because that pulls in `widgets/`) → `LibSharedMedia-3.0` (via its `lib.xml`) → **`LibKa0s`** (last, via its own `LibKa0s.xml`, which spells out `Core` → `Env` → `Pool` → `Item` → `Media` → `Widgets` → `DebugLog` → `Slash` → `Options` → `OptionsWidgets` → `OptionsCompose` → `OptionsScroll` → `Perf` → `PerfPanel`; every other major refuses to register without `Core`, which is why the folder is vendored whole).
