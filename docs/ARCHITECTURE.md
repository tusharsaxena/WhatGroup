...
Twelve rows, all profile-scoped, declared in `settings/Schema.lua` and valued in `defaults/Profile.lua`
...
`LibKa0s-Options-1.0`'s `RenderTabbedSchema` in declaration order — **General** (2), **Chat** (7),
**Popup** (3). No page banner (`options-ui-§14`): this addon has no per-window settings and no active
window for a banner to name. `section` is a different field and is unchanged by the tabs — it is
`/wg list`'s grouping key, which is why `notify.delay` is edited on General and lists under `notify`.

| Path | Type | Tab |
|---|---|---|
| `enabled` | bool | General — the master switch; its `onChange` off-flip calls `WhatGroup:WipeCapture()` |
| `notify.delay` | number | General — seconds to wait before notify **and** popup (`0` = instant) |
...
| `list` / `get` / `set` | library | The schema CLI, over the twelve rows above |
