...
Seventeen rows — sixteen profile-scoped and one session-only — valued in `defaults/Profile.lua`
(`NS.C`). Eleven are declared in `settings/Schema.lua`; the other six are the **Master controls**
...
| `list` / `get` / `set` | library | The schema CLI, over the seventeen rows above |
...
| `PLAYER_REGEN_DISABLED` + `PLAYER_REGEN_ENABLED` | `core/WhatGroup.lua` `OnEnable`, both to `OnCombatStateChanged` | Re-asks the `visibility` gate on the transition and applies the answer: hides a popup the gate has just closed on, shows one it has just opened for when a capture is still pending. The edge is taken from the **event name**, not from `InCombatLockdown()`, which can still read stale on the frame `PLAYER_REGEN_DISABLED` fires |
...
teleport cooldown countdown at `modules/Frame.lua:312`, a 1-second `ScheduleRepeatingTimer` added
...
armed it: a single handle, replaced rather than stacked, **armed only from the popup's `OnShow` and
only while `f:IsShown()`**, canceled from the popup's `OnHide`, from the top of every
`ConfigureTeleportButton` run, and by the tick that sees the cooldown reach zero. The arm and the
cancel are the same transition read in both directions, which is what makes "cannot outlive" a
structural claim rather than a list of remembered call sites.
