...
| Settings schema, panel wiring, helpers, db.profile shape | `settings/Schema.lua`, `settings/OptionsSetup.lua`, `settings/Panel.lua` | [docs/settings-system.md](./settings-system.md) |
| `/wg` slash UX + `COMMANDS` table | `settings/Slash.lua` | [docs/slash-dispatch.md](./slash-dispatch.md) |
| On-screen debug console + `NS.Debug` sink | `core/DebugLogSetup.lua` | [docs/debug-console.md](./debug-console.md) |
...
The teleport button's cooldown swipe (`modules/Frame.lua`) is the one thing in the addon that
animates, and it does not weaken that claim: a `CooldownFrameTemplate` is driven engine-side once
armed with `SetCooldown`, so the Lua side runs exactly once per popup open. Its companion
countdown text is deliberately **static** — computed when the popup is populated and never
refreshed — for the same reason. A ticking readout would need one of the three mechanisms above
and would re-arm the full `LibKa0s-Perf` wiring MUST; the swipe carries the live feedback instead.
