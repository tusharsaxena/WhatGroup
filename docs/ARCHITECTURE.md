...
Eighteen rows — sixteen profile-scoped and two session-only — valued in `defaults/Profile.lua`
(`NS.C`). Eleven are declared in `settings/Schema.lua`; the other seven are the **Master controls**
...
order — **Master controls** (7), **Chat** (8), **Popup** (3). The `Helpers.MASTER_GROUP` hook key,
