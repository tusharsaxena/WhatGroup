...
LFG events ─▶ capture pipeline ─▶ pendingInfo
  ...
  module (or, for Compat, wires the library's members) and the library owns the rest:
    ...
    core/DebugLogSetup.lua  ─►  LibKa0s-DebugLog-1.0  the console, formatters, buffer, report frame
...
| Diagnostics report (`/wg diagnostics`) — the sections and the two read-only accessors | `modules/Diagnostics.lua`, `core/DebugLogSetup.lua`, `core/WhatGroup.lua` (`CaptureSnapshot`), `modules/Frame.lua` (`NS.FrameSnapshot`) | [docs/debug.md](./debug.md) |
...
one `Hide` it owes, and the launcher menu grays every entry but Enabled while disabled. `tests/test_disabled.lua`
...
- **ONE launcher object, registered twice: left opens Settings, right opens the options menu.** `core/LauncherSetup.lua` builds a single LibDataBroker-1.1 object of `type = "launcher"` and hands THAT object to LibDBIcon-1.0 (launcher-§1): one `OnClick`, one icon, one label, one identity, so the minimap button and any broker display cannot drift apart. Both registrations use the addon's FOLDER name, because LibDBIcon keys the button's saved position by it; the object's **`label`** is the brand name in plain text, **`Ka0s WhatGroup`** (launcher-§1), which is what a broker display prints beside the other ten Ka0s rows — not the TOC `Title` and not the folder name. **Left-click opens the settings panel** through `WhatGroup:OpenSettings`, the `/wg config` body, in either state; **right-click opens the options menu** (`LibKa0s-Launcher-1.0` minor 4, launcher-§2, standard v2.67.0) — the client's own context menu, titled `Ka0s WhatGroup`, with four checkboxes, each wired to the body its verb runs: **Enabled** (`WhatGroup:SlashEnabled`, the `/wg enable` / `/wg disable` body), **Locked** (`WhatGroup:SlashToggleLock`, which is `/wg set locked toggle` — there is no `/wg lock` verb, the lock being a checkbox under slash-commands-§8's MAY), **Test mode** (`WhatGroup:SlashToggleTestMode`, bare `/wg test`) and **Show window** (`WhatGroup:ToggleFrame`, whose close arm is the same `dismissPopup` body the Close button and ESC run, with the checkmark read from `WhatGroup:IsFrameOnScreen`). While the addon is disabled the library grays all but Enabled. The launcher keeps no copy of any state, builds no menu of its own, and there is no setting that reassigns either button. Visibility is the one **global** schema row, `global.minimap.shown` (stored at `db.global.minimap.hide`; see `## Settings Schema`), and **it survives every reset this addon ships** — a property of the row rather than of its store (launcher-§3, standard v2.54.0), and true here without an exemption because all three reset surfaces land on one `db:ResetProfile()` body: see [docs/settings-panel.md → The minimap button row](./settings-panel.md#the-minimap-button-row). The broker object itself has no toggle, deliberately, because a display already offers the player one. **The hover tooltip is the library's** (`LibKa0s-Launcher-1.0` minor 3, launcher-§1): it draws the same block in every Ka0s addon, disabled or not — `Ka0s WhatGroup  v<TOC version>`, `Enabled`, `Locked` (the profile's `locked`, the Lock frame row), `Test mode` (`NS.State.testMode`), `Left-click: Open settings` and `Right-click: Options menu` — each state read on every hover. The descriptor only answers its questions (`version`, `isEnabled`, `isLocked`, `isTestMode`) and passes no `onTooltipShow`, since a host hook may only append lines of its own and this addon has none (anti-pattern #89).
...
| `debug-content.md` | What WhatGroup feeds the LibKa0s console: the descriptor, the tag vocabulary, the session flag, `/wg debug`. Tier 3 because it is the console's addon-owned content, while the Tier 2 `debug.md` covers the diagnostics report |
