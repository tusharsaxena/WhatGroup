...
teleport cooldown countdown at `modules/Frame.lua:428`, a 1-second `ScheduleRepeatingTimer` added
...
`local addonName, NS = ...` in the six files that read it (`core/CoreSetup.lua`,
`core/EnvSetup.lua`, `core/MediaSetup.lua`, `core/DebugLogSetup.lua`,
`core/WhatGroup.lua`, `settings/Panel.lua` — all six hand it to a vendored
library that cannot infer which folder it was copied into) and `local _, NS = ...`
