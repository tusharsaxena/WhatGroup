...
7. **`locales/enUS.lua`** — `NS.L`, a metatable shell whose missing keys return themselves. Every string the addon authors is routed through it (§8.3 / WG-07). Loads after `core/` per the layout load order, so callers reference `NS.L[...]` at runtime, never at file scope.
