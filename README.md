...
There is a minimap button too, wearing the addon's own logo: **left-click opens the popup** and clicking again puts it away, **right-click opens Settings**. Hover it and the tooltip says where things stand — **Enabled**, **Locked** (the popup's Lock frame) and **Test mode** — and it still answers while the addon is switched off, pointing you at `/wg enable`. The same entry turns up in Titan Panel, ElvUI data texts or any other broker display you run — it is one button shown in two places, not two. Untick **Minimap button** on the **Master controls** tab to hide it; that choice is remembered per installation, so switching profiles will not move your buttons around and **Reset all settings** will not bring a hidden one back.
