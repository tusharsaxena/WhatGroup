-- LibKa0s-Options-1.0 — the Blizzard settings-canvas shell: the panel factory, the lazy Defaults
-- button, and the reset/refresh trio every Ka0s addon's options UI runs on. The page registry it
-- attaches is OptionsRegistry.lua's.
--
-- Ten files, one major. This one is the shell; OptionsRegistry.lua the page registry, the
-- category it registers and its combat park; OptionsWidgets.lua is the schema-row -> AceGUI
...
--
-- Parking a combat-time CreateOptionsPanel for the end of combat -- lib.__parkedPanels,
-- lib.__OnParkEvent, lib.__parkRegistration and the private lib.__parkFrame -- is
-- OptionsRegistry.lua's from Options minor 26, beside CreateOptionsPanel, its one caller.
...
function lib:New(d)
  ...
  -- ── the page registry ────────────────────────────────────────────────────────────────────
  --
  -- O.RegisterOptionsPage, O.__pages, O.CreateOptionsPanel (with its combat park) and
  -- O.OpenOptionsPanel are OptionsRegistry.lua's from minor 26, attached here, where they used to
  -- be defined, so an instance gets its members in the same order. The registry's state (the page
  -- queue, the main category, the parked flag) went with them: nothing else here read it.
  if lib.__AttachRegistry then lib.__AttachRegistry(O, d) end
