...
--
-- Minors 2 and 3 also carried the combat lock's page chrome (options-ui-§2): the one event frame
-- the lock listens on, its dispatcher, the cover over a page and the level that puts it above the
-- strip. Minor 6 moved all of it, unchanged, to OptionsCombat.lua (LK-ATS-03); what stays here is
-- the refusal every control in this file asks, and the one call that attaches the cover.
--
...
function lib.__AttachTabs(O, d)
  ...
  -- ── the combat cover (OptionsCombat.lua from minor 6) ───────────────────────────────────
  -- O.__buildCover and O.__releaseOwnedFocus, attached where they used to be defined.
