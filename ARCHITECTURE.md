...
- **Parent settings category is a thin landing page.** WoW 12.0 hides parent-category widgets when the parent has subcategories, so the parent never carries schema widgets — instead it shows the logo, TOC notes, and the slash-command list. `/wg config` opens the **General subcategory** (`self._settingsCategory:GetID()`) so the user lands on the populated settings page rather than the landing page. See [docs/wow-quirks.md](./docs/wow-quirks.md#settings-api-parent-vs-subcategory).
