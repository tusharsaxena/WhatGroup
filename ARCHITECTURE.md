...
- **Parent settings category is intentionally empty.** WoW 12.0 hides parent-category widgets when the parent has subcategories. `/wg config` opens the **General subcategory** (`self._settingsCategory:GetID()`) so the user lands on the populated page. See [docs/wow-quirks.md](./docs/wow-quirks.md#settings-api-parent-vs-subcategory).
...
- **Lazy AceGUI panel build.** The General subcategory's body is built on first `OnShow` (via a `built` one-shot), and the AceGUI container's frame hooks `OnSizeChanged` to forward dimensions into AceGUI's layout. Without this, parented-to-Blizzard containers stay at 0×0. See [docs/wow-quirks.md](./docs/wow-quirks.md#lazy-acegui-panel-build).
