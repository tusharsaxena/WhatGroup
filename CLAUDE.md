...
- **Never edit `libs/` or `tests/_kit/`.** Both are whole-folder, byte-identical copies of
  ...
  reverts it. The addon takes seven of LibKa0s's majors (Core, DebugLog, Env, Launcher,
  Media, Options, Slash) through the seven seam files `core/CoreSetup.lua`,
  `core/EnvSetup.lua`, `core/MediaSetup.lua`, `core/DebugLogSetup.lua`,
  `core/LauncherSetup.lua`, `settings/OptionsSetup.lua` and `settings/Slash.lua`; **Perf is
  declined** on structural grounds ([`LIBKA0S-15`](https://github.com/tusharsaxena/WhatGroup/issues/7)).

## Bundled LibKa0s

Bundles [LibKa0s](https://github.com/tusharsaxena/LibKa0s) v1.44.0 (MIT). That line is the
